...
#include "types_ladd.h"  /* type definitions */
#include "parser_ladd.c" /* parser */
...
void deOrderEdgeByRandom(nodeP *np,PreprocData *pd)
{
...
void collectCutEdges_step1(cType curN, PreprocData *pd)
{
  // printf("call markcut in %ld\n",curN);
  nodeP* nodes = pd->gd->nodes;
  assert(curN <= pd->gd->N && curN >= 1);
...
// printf("call markcut in %ld\n",curN);
  ...
  for (int ni = 0; ni < cnt; ni++)
  {
    ...
    //if u is 0, this also is not satisified
    if(pd->gpdep[zn] < pd->gpdep[u] ){
      ...
      gMe[zn] = min(gMe[zn], pd->gd->N - pd->gpnc[s]); // it is still safe when s == 0
    }
    else if(pd->gpdep[zn] == *curDep + 1){
      //child
      collectCutEdges_step2(zn,pd);
    }
...
void preProc(PreprocData *pd){
  ...
  for (int ipass = 0; ipass < pd->total; ipass++)
  {
    ...
    pd->gprc = allResults[ipass].pacc_upid; //me
    pd->gpaccmcv = allResults[ipass].pacc_upmincv; //already used as tree id
    pd->gprn = allResults[ipass].pacc_pos_upmincv; //rn
    ...
    */
    ...
    // memset(allResults[0].pacc_jointid, 0, len * sizeof(cType));
...
sType* idxs = NULL;
   ...
   long usedEdgeCountDegree = 0; //选中的边中degree <K的点相关的边
   long totalEdgeCountDegree = 0; //选中的边中degree <K的点相关的边
   long totalCost = 0;
...
void buildMCT(PreprocData *pd){
    ...
    for(int i=1; i<=N; i++){
      if(pd->gpaccmcv[i] > 0){
        ...
        buildMCT_cost0Tree(i,pd, i); //先连接非侯选边，侯选边放estack中
      }
    }
    ...
    //边从小到大排序
    HeapSort2(idxs, estack, stackTop);  //按 cost排序
    ...
    */    
    ...
    for(int k=0; k<stackTop; k++){
      ...
      //assert(estack[i] ->cost > 0);
      //gpaccmcv[n] is the tree id node n which is also a node id

      cType n1 = estack[i]->endNode;
      cType n2 = estack[i]->rev->endNode;
      cType t1 = n1;
      //every node has non-zero gpaccmcv
      ...
      if(t1 != t2){
        ...
        if(estack[i]->w != (MARK_CUT_SHIELD + 1)){ // if MARK_CUT_SHILED+1 it is already handled by small component node
    ...
    printf("c K %ld, total, chosen, cost is %ld(%ld) %ld(%ld) %ld;  all lessK neighbor edge %ld\n",pd->K,total_cut_edge,totalEdgeCountDegree,usedEdgeCount,usedEdgeCountDegree,totalCost,total_cut_edge_lessK);

}



void printGraphWithShieldedEdges(PreprocData *pd){
  printf("p max %ld %ld\n",pd->gd->N, pd->gd->M);
  int cap = 1;
  for(cType curN = 1; curN<=pd->gd->N; curN++){
    //只输出curN连到id比自己大的
...
void heuristicPreprocess(PreprocData *pd, int gType, int gDegree){
  ...
  memset(harrEdge, 0, 250 * 100000*  sizeof(edgeP*));
  for(cType curN = 1; curN<=pd->gd->N; curN++){
    //只输出curN连到id比自己大的
    ...
    for(int ni=0; ni<curCnt; ni++){
      ...
      if(n2 > curN){
        ...
        harrEdge[a-1][b-1][cost-1][harrIdx[a-1][b-1][cost-1]] = ep;
  ...
  printf("c toCheckTotalCost %ld, toCheckTotalEdge %ld, totalTmp1 %ld\n",toCheckTotalCost,toCheckTotalEdge,totalTmp1);
  ...
  if(test_all_cost != parse_all_cost || toCheckTotalEdge/2 != pd->gd->M){
    printf("test_all_cost != parse_all_cost or toCheckTotalEdge != M\n");
    fflush(stdout);
    exit(0);
  }
    ...
    (1,2)(2,2)(2,3)(3,3)
    */
// void runheuristic_LeastDegree(PreprocData *pd)
// {
//   long allcost = 0;
...
//         for (int k = 0; k < 10; k++)
//         {
//           printf(" i %d j %d k %d idx %ld\n", i, j, k, harrIdx[i - 1][j - 1][k]);
//           for (int idx = 0; idx < harrIdx[i - 1][j - 1][k]; idx++)
//           {
//             if (allcost > totalCost)
//             {
//               printf("get enough edges\n");
//               // exit(0);
//               return;
//             }
//             edgeP *ep = harrEdge[i - 1][j - 1][k][idx];
//             allcost += (k + 1);
//             ep->w = ep->rev->w = MARK_CUT_SHIELD;
//           }
//         }
//       }
//     }
...
//   assert(allcost >= totalCost);
//   //这里不需要allcost超过，只需要找小的边就行
//   printf("here allcost %ld  totalCost %ld\n",allcost,totalCost);
// }
...
*/
// //按总和选择范围，再随机挑选
// edgeP* harrEdgeUnique[5*5*10*200000];
// void runheuristic_RandomLeastDegree(PreprocData *pd)
// {
//   long allcost = 0;
//   long aidx = 0;
//   edgeP* ep = NULL;
...
//   }
//   else{
//     printf(" totalcost %ld allcost %ld\n",totalCost,allcost);
//   }

//   long toShieldCost = 0;
//   long sidx = 0;
//   while(toShieldCost < totalCost){
//     if(aidx <= 0){
//       printf("not enough edges here2\n");
//       exit(0);      
//     }
//     //选择边
//     sidx = ((cType)rand())%aidx;
//     ep = harrEdgeUnique[sidx];
//     long cost = ep->cost; 
//     toShieldCost += cost;
//     ep->w = ep->rev->w = MARK_CUT_SHIELD;
//     //交换
//     harrEdgeUnique[sidx] = harrEdgeUnique[aidx-1];
//     aidx --;
...
//只看最小的那边
// void runheuristic_LeastDegreeSingle(PreprocData *pd)
// {
//   long allcost = 0;
...
//按总和选择范围，再随机挑选
// void runheuristic_RandomLeastDegreeSingle(PreprocData *pd)
// {
//   long allcost = 0;
//   long aidx = 0;
//   edgeP *ep = NULL;
...
// void runheuristic_LeastCost(PreprocData *pd)
// {
//   long allcost = 0;
...
//   printf("here lcost allcost %ld  totalCost %ld\n",allcost,totalCost);

// }


//按总和选择范围，再随机挑选
// void runheuristic_RandomLeastCost(PreprocData *pd)
// {
//   long allcost = 0;
//   long aidx = 0;
//   edgeP *ep = NULL;
...
void connTwoNodes(PreprocData *pd, cType head, cType tail){
    ...
    if(np->orderedEdges != NULL) {
      free(np->orderedEdges);
      np->orderedEdges = (sType *)walloc(np->nIdx + 1, sizeof(sType));
    }
...
cType _justConCandiPair(PreprocData *pd, cType n1, cType n2){
  ...
  {  
      // printf("conn n1 %ld with n2 %ld when N is %ld\n",n1,n2,pd->gd->N);
      //judge whether they are connected already
  ...
  cType totalLessUpBoundCount = 0;
    ...
    //printf("c small deg node %ld\n",curN);      
      ...
      for (int ni = 0; ni < (pd->gd->nodes+curN)->nIdx; ni++)
      {
        ...
        //printf("c unmark edge due to small deg: (%ld, %ld)\n",curN, eh->endNode);
        eh->w = eh->rev->w = MARK_CUT_SHIELD+1;

          // usedEdgeCount --;      

        // if(eh->w == MARK_CUT_SHIELD){
        //   assert(eh->rev->w == MARK_CUT_SHIELD);  
...
/////将选中的边连接起来
cType cycleConnShieldEdges(PreprocData *pd){
  cType segArr[1000000][2]; //store the path or edge
  int segArrIdx = 0;
  sType nodeSign[1100000];
  memset(nodeSign, 0,1100000*sizeof(sType));

  sType tcnt = 0;
  //check edge in stack where w == MARK_CUT_SHIELD, which is the edges to shield
  while(stackTop >0){
    ...
    // printf("not small deg node edge (%ld, %ld)\n",end1,end2);
    // assert(nodeSign[end1] == 0);
    // assert(nodeSign[end2] == 0);

    //now we have a path [end1 and end2], pushed into some place and wati for final conn edges and paths
  ...
  assert(tcnt == usedEdgeCount);
  if(segArrIdx < 2){
    printf("c warning: segArrIdx %d <2 \n", segArrIdx);
  }


  // assert(segArrIdx >= 2); //single need other logics

  //conn that in segArr head by tail
  cType fCnt = 0;
  //conn out node
  ...
  //cycle conn
  // for(int i = 0 ; i < segArrIdx; i++){
  //   connTwoNodes(pd,segArr[i][1], segArr[(i+1)%segArrIdx][0]);
  //   fCnt ++;
  // }
